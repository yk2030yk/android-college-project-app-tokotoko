...
public class GuideGeneralDetailsFragment extends Fragment {
	...
	public void onStart() {
				...
				PostServerTask task = new PostServerTask(urlcreator.createHotelURL()) {
					...
					protected void onPostExecute(Boolean result) {
						...
						if (list.size() > 0) {
							...
							GuideActivity act = (GuideActivity)getActivity();
							if (act != null) {
								act.setHotel(h);
							}
		...
		TextView memoButton = (TextView)frgLayout.findViewById(R.id.textView_memo);
		memoButton.setOnClickListener(new View.OnClickListener() {
			
			@SuppressLint("CutPasteId")
			@Override
			public void onClick(View v) {
				Intent i = new Intent(getActivity(), EditGuideMemoActivity.class);

...
public class GCMIntentService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (!extras.isEmpty()) {
			if (GoogleCloudMessaging.MESSAGE_TYPE_SEND_ERROR.equals(messageType)) {
				...
				// kind=0:自分, kind=1:メンバー, kind=2:観光地,
				// kind=3:グルメ
	...
	private void sendNotification(String msg, int kind) {
		...
		NotificationCompat.Builder builder = new NotificationCompat.Builder(this).setSmallIcon(imageId).setContentTitle("メッセージ").setStyle(new NotificationCompat.BigTextStyle().bigText(msg)).setContentText(msg);

<?php
    ...
    if (spot_id > 0 || spot_id != null) {
        mysql_set_charset('utf8');
        $result = mysql_query('select spot.id as spot_id, spot.name as spot_name, address, exp, tel, access, sample_rate, opening_time, sub_category.name as cate_name, area.name as area_name, lat, lng , image from spot left join sub_category on spot.sub_cate_id = sub_category.id left join area on spot.area_id = area.id where spot.id='.$spot_id.';');
        if (!$result) {
        ...
        while ($row = mysql_fetch_assoc($result)){
            ...
            $content->appendChild($dom->createElement("Access", $row["access"]));

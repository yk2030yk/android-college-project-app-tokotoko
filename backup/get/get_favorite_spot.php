<?php
    ...
    $result = mysql_query('select * from favorite left join (select spot.id as spot_id, spot.name as spot_name, address, exp, tel, sample_rate, opening_time, sub_category.name as cate_name, area.name as area_name, lat, lng , image from spot left join sub_category on spot.sub_cate_id = sub_category.id left join area on spot.area_id = area.id) as spot on spot.spot_id = favorite.spot_id;');
    if (!$result) {
